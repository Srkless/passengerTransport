...
void gui::EnterLocation(std::string username, std::string name)
{
	...
	int t = 0;
	auto backButton = ftxui::Button("Back", [&] {IsEqual(data, path, name), gui::createCodeLocation(username); });

	auto Enter = ftxui::Button("Enter", [&] {writeLocation(country, location, path, data), gui::EnterLocation(username, name), t = 0; });
...
void gui::createCodeBooksInterface(std::string username)
{
	...
	ftxui::Color bannerMessageColor = blue;
	auto Location = ftxui::Button("Create Location codebook", [&] {createCodeLocation(username); });//Dodati interfejs
	auto Bus = ftxui::Button("Create Bus codebook", [&] {createCodeBus(username); });//Dodati interfejs
	auto Tour = ftxui::Button("Create Tour codebook", [&] {createCodeTour(username); });//Dodati interfejs
	auto Delete = ftxui::Button("Delete Codebook", [&] {gui::DeleteCodeBooks(username); });//Dodati interfejs
